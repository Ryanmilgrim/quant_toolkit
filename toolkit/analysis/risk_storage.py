...
class RiskModelSnapshotInfo:
    """Lightweight metadata for listing saved risk model snapshots."""
...
def save_factor_snapshot(
    ...
    *,
    ...
    """Save a risk model snapshot to a single pickle file."""
...
def load_factor_snapshot(path: Path) -> RiskModelSnapshot:
    """Load a risk model snapshot from a pickle file."""
    with path.open("rb") as handle:
        obj = pickle.load(handle)
    if not isinstance(obj, RiskModelSnapshot):
        raise TypeError("Invalid snapshot payload.")
    return obj


def list_factor_snapshots(directory: Path) -> list[RiskModelSnapshotInfo]:
    """List saved risk model snapshots in a directory."""
    if not directory.exists():
        return []
    infos: list[RiskModelSnapshotInfo] = []
...
FactorAnalysisSnapshot = RiskModelSnapshot
FactorSnapshotInfo = RiskModelSnapshotInfo
